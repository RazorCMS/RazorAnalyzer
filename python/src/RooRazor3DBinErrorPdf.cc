...
//---------------------------------------------------------------------------
void RooRazor3DBinErrorPdf::setTH3Binning(TH3* _Hnominal, TH3* _Herror){
  ...
  for (Int_t i=0; i<xBins; i++){
	...
	if (_Herror)
	  errArray[th1xBin] = _Herror->GetBinContent(i+1,j+1,k+1);
	else	  
	  errArray[th1xBin] = 0.;
...
Double_t RooRazor3DBinErrorPdf::evaluate() const
{
  ...
  if (total_integral>0.0) {
    return pow(histArray[iBin],errArray[iBin]) * integral;
  } else return 0;
   ...
   if(code==1) { 
	 ...
	 if (zCut >= zLow and zCut < zHigh){
	   ...
	   if(xHigh <= xCut && yHigh <= yCut) integral += 0.0;
	     ...
	     integral += pow(histArray[iBin],errArray[iBin]) * ( Gfun(xCut,yLow)-Gfun(xCut,yHigh)-Gfun(xHigh,yLow)+Gfun(xHigh,yHigh) );
	   }
	   else if(yLow < yCut && yHigh > yCut && xHigh <= xCut) {
	     integral += pow(histArray[iBin],errArray[iBin]) * ( Gfun(xLow,yCut)-Gfun(xLow,yHigh)-Gfun(xHigh,yCut)+Gfun(xHigh,yHigh) );
	   }
	   else if(xLow < xCut && xHigh > xCut && yLow < yCut && yHigh > yCut) {
	     integral += pow(histArray[iBin],errArray[iBin]) * ( -Gfun(xLow,yHigh)-Gfun(xHigh,yLow)+Gfun(xHigh,yHigh)+Gfun(xLow,yCut)+Gfun(xCut,yLow)-Gfun(xCut,yCut) );
	   }
	   else integral += pow(histArray[iBin],errArray[iBin]) * ( Gfun(xLow,yLow)-Gfun(xLow,yHigh)-Gfun(xHigh,yLow)+Gfun(xHigh,yHigh) );
	 }
       }
     }
   } else {
