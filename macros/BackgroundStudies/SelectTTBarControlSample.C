...
void RunSelectDileptonTTBarControlSample( vector<string> inputfiles, vector<string> processLabels, int option = -1, string label = "") {
...
void RunSelectSingleLeptonTTBarControlSample( vector<string> inputfiles, vector<string> processLabels, int option = -1, string label = "") {
  ...
  for (uint i=0; i < inputfiles.size(); ++i) {
	...
	continue;
      ...
      //Options
      //******************************
      //1 or more CSV Medium tag
      if (option == 0 &&
	  !(events->NBJetsMedium >= 1)
	  ) continue;
      
      //2 or more CSV Loose tag
      if (option == 1 &&
	  !(events->NBJetsLoose >= 2)
	  ) continue;
  ...
  if (stackMR->GetHists()->GetEntries() > 0) {
    ...
    stackMR->GetHistogram()->GetYaxis()->SetTitle(((TH1F*)(stackMR->GetHists()->At(0)))->GetYaxis()->GetTitle());
    legend->Draw();
    cv->SaveAs(Form("Razor_TTBarCrossCheckRegion_MR%s.gif",Label.c_str()));
  }
  ...
  if (stackRsq->GetHists()->GetEntries() > 0) {
    ...
    stackRsq->GetHistogram()->GetYaxis()->SetTitle(((TH1F*)(stackRsq->GetHists()->At(0)))->GetYaxis()->GetTitle());
    legend->Draw();
    cv->SaveAs(Form("Razor_TTBarCrossCheckRegion_Rsq%s.gif",Label.c_str()));
  }
  ...
  if (stackMT->GetHists()->GetEntries() > 0) {
    ...
    legend->Draw();
    cv->SaveAs(Form("Razor_TTBarCrossCheckRegion_MT%s.gif",Label.c_str()));
  }
  ...
  histMRVsRsq[0]->SetStats(false);
  histMRVsRsq[0]->Draw("colz");
  cv->SaveAs(Form("Razor_TTBarCrossCheckRegion_EventCountsTTBar_MRVsRsq%s.gif",Label.c_str()));

  cv = new TCanvas("cv","cv", 800,600);
  statUnc_MRVsRsq->SetStats(false);
  statUnc_MRVsRsq->Draw("colz");
  cv->SaveAs(Form("Razor_TTBarCrossCheckRegion_StatUncTTBar_MRVsRsq%s.gif",Label.c_str()));
 ...
 //--------------------------------------------------------------------------------------------------------------
  // Output
  //==============================================================================================================
  TFile *file = TFile::Open(("TTBarCrossCheckRegionPlots"+Label+".root").c_str(), "UPDATE");
  file->cd();
  ...
  file->WriteTObject(statUnc_MRVsRsq,"statUnc_MRVsRsq_TTBar","WriteDelete");
  cout << "here7\n";
...
void SelectTTBarControlSample( int option = -1, string label = "") {
  ...
  inputfiles.push_back("/afs/cern.ch/work/s/sixie/public/Run2SUSY/RazorControlRegions/skim/RazorControlRegions_DileptonSkim_TTJets_25ns_weighted.root");  
  inputfiles.push_back("/afs/cern.ch/work/s/sixie/public/Run2SUSY/RazorControlRegions/skim/RazorControlRegions_DileptonSkim_DYJetsToLL_HT100ToInf_25ns_weighted.root");
  inputfiles.push_back("/afs/cern.ch/work/s/sixie/public/Run2SUSY/RazorControlRegions/skim/RazorControlRegions_DileptonSkim_WJetsToLNu_HT100ToInf_25ns_weighted.root");
  inputfiles.push_back("/afs/cern.ch/work/s/sixie/public/Run2SUSY/RazorControlRegions/skim/RazorControlRegions_DileptonSkim_SingleTop_25ns_weighted.root"); 
  ...
  //*********************************************************************
  //RunSelectDileptonTTBarControlSample(inputfiles,processLabels,-1,"all");
  //RunSelectDileptonTTBarControlSample(inputfiles,processLabels,0,"emu");
  //RunSelectDileptonTTBarControlSample(inputfiles,processLabels,1,"eemumu");
  //RunSelectDileptonTTBarControlSample(inputfiles,processLabels,2,"ee");
  //RunSelectDileptonTTBarControlSample(inputfiles,processLabels,3,"mumu");
  ...
  RunSelectSingleLeptonTTBarControlSample(inputfiles,processLabels,0,"1CSVMTag");
  RunSelectSingleLeptonTTBarControlSample(inputfiles,processLabels,1,"2CSVLTag");
