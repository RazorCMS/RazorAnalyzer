...
EnergyScaleCorrection_class_2017::EnergyScaleCorrection_class_2017(std::string correctionFileName, unsigned int genSeed):
...
float EnergyScaleCorrection_class_2017::ScaleCorrection(unsigned int runNumber, bool isEBEle,
...
float EnergyScaleCorrection_class_2017::ScaleCorrectionUncertainty(unsigned int runNumber, bool isEBEle,
							      double R9Ele, double etaSCEle, double EtEle, unsigned int gainSeed, std::bitset<scAll> uncBitMask) const
{
  
  const correctionValue_class_2017& c = getScaleCorrection(runNumber, isEBEle, R9Ele, etaSCEle, EtEle,gainSeed);
...
correctionValue_class_2017 EnergyScaleCorrection_class_2017::getScaleCorrection(unsigned int runNumber, bool isEBEle, 
								      double R9Ele, double etaSCEle, double EtEle, unsigned int gainSeed) const
{

  // buld the category based on the values of the object
  correctionCategory_class_2017 category(runNumber, etaSCEle, R9Ele, EtEle, gainSeed);
  correction_map_t_2017::const_iterator corr_itr = scales.find(category); // find the correction value in the map that associates the category to the correction
  ...
  if(corr_itr == scales.end()) { // if not in the standard classes, add it in the list of not defined classes
    ...
    // 	correctionValue_class_2017 corr;
 ...
 *
 */
void EnergyScaleCorrection_class_2017::ReadFromFile(std::string filename)
{
...
void EnergyScaleCorrection_class_2017::AddScale(TString category_, int runMin_, int runMax_, double deltaP_, double err_deltaP_, double err_syst_deltaP, double err_deltaP_gain)
{
  
  correctionCategory_class_2017 cat(category_); // build the category from the string
  cat.runmin = runMin_;
  cat.runmax = runMax_;
 ...
 if(scales.count(cat) != 0) { 
  ...
  correctionValue_class_2017 corr; // define the correction values
  corr.scale = deltaP_;
...
//============================== SMEARING
void EnergyScaleCorrection_class_2017::AddSmearing(TString category_, int runMin_, int runMax_,
					      double rho, double err_rho, double phi, double err_phi,
					      double Emean, double err_Emean)
{
  
  correctionCategory_class_2017 cat(category_);
...
void EnergyScaleCorrection_class_2017::ReadSmearingFromFile(std::string filename)
{
#ifdef PEDANTIC_OUTPUT
...
float EnergyScaleCorrection_class_2017::getSmearingSigma(int runNumber, bool isEBEle, 
						    ...
						    float R9Ele, float etaSCEle, float EtEle, unsigned int gainSeed, float nSigma_rho, float nSigma_phi) const
{
  ...
  correctionCategory_class_2017 category(runNumber, etaSCEle, R9Ele, EtEle, 0);
  correction_map_t_2017::const_iterator corr_itr = smearings.find(category);
...
float EnergyScaleCorrection_class_2017::getSmearingRho(int runNumber, bool isEBEle, float R9Ele, float etaSCEle, float EtEle, unsigned int gainSeed) const
{
...
bool correctionCategory_class_2017::operator<(const correctionCategory_class_2017& b) const
{
...
correctionCategory_class_2017::correctionCategory_class_2017(TString category_)
{
  ...
  //LOGDRESSED("EnergyScaleCorrection_class_2017") << "p1 = " << p1 << "\t" << std::string::npos << "\t" << category.size();
  //LOGDRESSED("EnergyScaleCorrection_class_2017") << etmin << "\t" << etmax << "\t" << category.substr(p1+1, p2-p1-1) << "\t" << p1 << "\t" << p2;
  ...
  if(p1 != std::string::npos) {
    ...
    //LOGDRESSED("EnergyScaleCorrection_class_2017") << etmin << "\t" << etmax << "\t" << category.substr(p1 + 1, p2 - p1 - 1);
  }
