...
void RazorControlRegions::Analyze(bool isData, int option, string outputfilename, string label)
{
    ...
    if (treeTypeOption == 1)
        ...
        events->CreateTree(ControlSampleEvents::kTreeType_PhotonAdd2MET_Full);
    else if (treeTypeOption == 15)
...
//                }
        ...
        for(int i = 0; i < nJets; i++){
            ...
            //NB: Currently the code excludes ALL identified "Good" leptons and photons from the jet collection
            ...
            for(auto& pho : GoodPhotons){
                double thisDR = deltaR(jetEta[i],jetPhi[i],pho.Eta(),pho.Phi());
                if(dR < 0 || thisDR < dR) dR = thisDR;
            }
            if(dR > 0 && dR < 0.4) continue; //jet matches a selected photon
            
//            // Skip the jet matched a gen photon
//            if (!isData)
//            {
//                for(int g = 0; g < nGenParticle; g++)
//                    if ((gParticleStatus[g] == 1 || gParticleStatus[g] == 23) && (gParticleId[g] == 22)) // final state gen particle is a photon
//                        if ((deltaR(gParticleEta[g] , gParticlePhi[g], jetEta[i], jetPhi[i]) < 0.1)) continue;
//            }
            if (printSyncDebug)  {
        ...
        //***********************************************************************************
        //Compute the razor variables using the selected jets, good leptons, and good photons
        //***********************************************************************************
        ...
        for(auto& pho : GoodPhotons) GoodPFObjects.push_back(pho);
        
        //sort good PF Objects
        sort(GoodPFObjects.begin(), GoodPFObjects.end(), greater_than_pt());
        ...
        if(GoodPhotons.size()>0){
            ...
            // GoodJets collections already veto GoodPhotons. However, historically razor susy inclusive analysis did not reject GoodPhotons to make the GoodJets
            // list, ie., GoodJets contains single photons. To keep thing this way, I merge GoodPhotons and GoodJets and then subtract the leading photon in the
            // GoodPhotons list from the merged list.
            vector<TLorentzVector> GoodJetsNoLeadPhoton = GoodJets;
            GoodJetsNoLeadPhoton.insert(GoodJetsNoLeadPhoton.end(), GoodPhotons.begin(), GoodPhotons.end()); // Merge GoodJets and GoodPhotons
        ...
        if (razorSkimOption == 1) {
            if ( !( 
                        (events->MR > 1000 && events->Rsq > 0.) || GoodJets.size() >= 20
                  )
               ) passSkim = false;
        }
        ...
        {
            if (!(
                        events->MR_NoPho > 1000 && events->Rsq_NoPho > 0.
                 )
               ) passSkim = false;	
        }
