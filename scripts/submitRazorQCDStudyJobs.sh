...
do
	...
	if [[ ! -e ${outputDir}${sample}.Job${jobnumber}Of${maxjob}.root ]]; then
	    ...
	    echo bsub -q 8nm -J RazorQCDStudy_${jobnumber} `pwd`/runRazorQCDJob_CERN_EOS.sh ${CMSSW_BASE} RazorQCDStudy ${inputfilelist} ${isData} 0 ${filesPerJob} ${jobnumber} ${sample}.Job${jobnumber}Of${maxjob}.root ${outputDir}
	    bsub -q 8nm -J RazorQCDStudy_${jobnumber} `pwd`/runRazorQCDJob_CERN_EOS.sh ${CMSSW_BASE} RazorQCDStudy ${inputfilelist} ${isData} 0 ${filesPerJob} ${jobnumber} ${sample}.Job${jobnumber}Of${maxjob}.root ${outputDir}
	    sleep 0.1
	fi
    done
done
